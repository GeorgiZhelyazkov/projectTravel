...
export default function RecentScreen() {
  ...
  const loadRecentItems = async () => {
    try {
      const items = await AsyncStorage.getItem('recentItems');
  ...
  // Refresh recent items when screen comes into focus
  useFocusEffect(
    useCallback(() => {
      loadRecentItems();
    }, [])
  ...
  const removeRecentItem = async (id: string) => {
    try {
  ...
  const renderItem = ({ item }: { item: StorageItem }) => (
    ...
    >
      ...
      <TouchableOpacity
        ...
        onPress={() => removeRecentItem(item.id)}
      >
  ...
  return (
    <View style={[styles.container, isDarkMode && styles.darkContainer]}>
      <FlatList
        ...
        ListEmptyComponent={
          <Text style={[styles.emptyText, isDarkMode && styles.darkText]}>Няма наскорошни маршрути</Text>
        }
      />
    </View>
  );
